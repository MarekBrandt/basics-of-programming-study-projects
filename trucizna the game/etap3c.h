...
void wczytajStanGry()
{
	...
	int tablicaIlosciKartWKociolku[2 * maxLiczbaGraczy] = { 0 };

	fgets(activePlayer, maxLiczbaLiterZdania, stdin); //wczytuje pierwsze dwie linie
	fgets(playersNumber, maxLiczbaLiterZdania, stdin);//z drugiej bierze liczbe graczy

	int liczbaGraczy = znajdzLiczbe(playersNumber, 0); // wyluskuje liczbe graczy z tekstu
	char kolory[maxLiczbaKolorow][maxLiczbaLiterKoloru];
	int ileKolorow = 0; //nie licze zielonego	
	...
	for (int i = 0; i < 2 * liczbaGraczy; i++)//wczytuje po lini dane, liczy ilosc kart i kolorow
	{
		...
		ileKolorow = policzKolory(activePlayer, kolory, ileKolorow, 20); //nie licze zielonego	
	}
	...
	{
		fgets(activePlayer, maxLiczbaLiterZdania, stdin);
		tablicaIlosciKartWKociolku[i] = znajdzWszystkieLiczby(activePlayer, 2);
	}
...
int policzKolory(char zdanie[], char kolor[][maxLiczbaLiterKoloru], int ileKolorow, int poczatek)//liczy kolory, ale odrzuca zielony
{
